...
async def get_token_statistics(db) -> dict:
...
async def get_cleanup_statistics() -> Dict[str, Any]:
    ...
    try:
        async with async_session_maker() as db:
            stats = await get_token_statistics(db)
            return {
                "success": True,
                "statistics": stats,
...
async def manual_token_cleanup(
    ...
    try:
        if dry_run:
            ...
            stats_result = await get_cleanup_statistics()
