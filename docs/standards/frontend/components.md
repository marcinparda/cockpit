...
### Primitives
Shared component primitives live in `@cockpit-app/shared-react-ui` (shadcn/ui-style, Radix UI + Tailwind + CVA). Add new primitives there, not per-app — see `libs/shared/ui/react/README.md` for the add-component process.

### Page-Level Blocks
Source page-level blocks from [shadcnui-blocks](https://www.shadcnui-blocks.com/blocks): inspect the preview HTML (Playwright or devtools), then adapt using primitives from the shared lib — never hand-roll a block that already exists there.

### Dark Mode
System preference detected in `index.html` via `prefers-color-scheme`, applied as a `.dark` class on root. All components use CSS variables that swap under `.dark` — never hardcode light-only colors in component styles.
