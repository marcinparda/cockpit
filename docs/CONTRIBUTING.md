...
5. **Is it the record of one specific task's work?** → `docs/tasks/<type>/<date>-<slug>/`, per the `grill-me` → `plan-slices` → `fresh-review` flow in [`onboarding.md`](../onboarding.md).
...
- `docs/standards/INDEX.md` must be updated whenever a standards file is added, removed, or its scope changes materially — it's the entry point, keep it accurate.
